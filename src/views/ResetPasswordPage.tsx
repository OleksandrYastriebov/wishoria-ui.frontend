            ...
            {!token ? (
              <motion.div key="invalid" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <div className="bg-[#111118] rounded-2xl border border-white/[0.07] shadow-2xl shadow-black/40 p-8 text-center">
                  <div className="flex items-center justify-center w-12 h-12 rounded-full bg-red-500/10 border border-red-500/20 mx-auto mb-4"><AlertTriangle size={22} className="text-red-400" /></div>
                  <h1 className="text-xl font-bold text-white mb-2">Invalid link</h1>
                  <p className="text-sm text-[#9898b4] mb-6">This reset link is missing or malformed. Please request a new one.</p>
                  <Link href="/forgot-password" className="text-sm text-violet-400 font-medium hover:text-violet-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 rounded">Request new link</Link>
                </div>
              </motion.div>
            ) : !succeeded ? (
              <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
                <div className="bg-[#111118] rounded-2xl border border-white/[0.07] shadow-2xl shadow-black/40 p-8">
                  ...
                  <p className="text-sm text-[#9898b4] mb-6">Choose a strong password of at least 8 characters.</p>
                  ...
                  <p className="text-sm text-[#9898b4] text-center mt-5">
                    <Link href="/sign-in" className="text-violet-400 font-medium hover:text-violet-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 rounded inline-flex items-center gap-1"><ArrowLeft size={13} />Back to sign in</Link>
                  </p>
                </div>
              </motion.div>
            ) : (
              <motion.div key="success" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                <div className="bg-[#111118] rounded-2xl border border-white/[0.07] shadow-2xl shadow-black/40 p-8 text-center">
                  <div className="flex items-center justify-center w-12 h-12 rounded-full bg-violet-500/10 border border-violet-500/20 mx-auto mb-4"><CheckCircle size={22} className="text-violet-400" /></div>
                  <h1 className="text-xl font-bold text-white mb-2">Password updated!</h1>
                  <p className="text-sm text-[#9898b4] mb-6">Your password has been changed. You can now sign in with your new credentials.</p>
                  <Link href="/sign-in" className="text-sm text-violet-400 font-medium hover:text-violet-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 rounded inline-flex items-center gap-1"><ArrowLeft size={13} />Sign in</Link>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
