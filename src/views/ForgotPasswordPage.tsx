            ...
            {!submitted ? (
              <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
                <div className="bg-[#111118] rounded-2xl border border-white/[0.07] shadow-2xl shadow-black/40 p-8">
                  ...
                  <p className="text-sm text-[#9898b4] mb-6">Enter your email and we'll send you a reset link.</p>
              ...
              <motion.div key="success" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.3 }}>
                <div className="bg-[#111118] rounded-2xl border border-white/[0.07] shadow-2xl shadow-black/40 p-8 text-center">
                  <div className="flex items-center justify-center w-12 h-12 rounded-full bg-violet-500/10 border border-violet-500/20 mx-auto mb-4"><MailCheck size={22} className="text-violet-400" /></div>
                  <h1 className="text-xl font-bold text-white mb-2">Check your inbox</h1>
                  <p className="text-sm text-[#9898b4] mb-1">If <span className="text-white font-medium">{submittedEmail}</span> is registered, you'll receive a password reset link shortly.</p>
                  <p className="text-xs text-[#55556e] mb-6">The link expires in 1 hour.</p>
                  <Link href="/sign-in" className="text-sm text-violet-400 font-medium hover:text-violet-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 rounded inline-flex items-center gap-1"><ArrowLeft size={13} />Back to sign in</Link>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
